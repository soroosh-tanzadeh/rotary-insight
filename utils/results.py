...
def _write_csv_train_validation(csv_path: str, all_results: Dict[str, Any]) -> None:
    rows: List[Dict[str, Any]] = []
    for model_name, res in (all_results.get("models", {}) or {}).items():
            ...
            rows.append(
                {
                    ...
                    "type": "split",
                    "index": idx,
                    "validation_accuracy": d.get("validation_accuracy"),
                    "validation_loss": d.get("validation_loss"),
                    "macro_precision": macro["precision"],
                    "macro_recall": macro["recall"],
                    "macro_f1": macro["f1"],
                    "auroc_macro_mean": (
                        (sum(auroc_list) / len(auroc_list)) if auroc_list else None
                    ),
                }
            )
        # Aggregate row
        ...
        rows.append(
            {
                ...
                "type": "aggregate",
                "index": "all",
                "validation_accuracy_mean": (agg.get("validation_accuracy") or {}).get(
                    "mean"
                ),
                "validation_accuracy_ci_low": (
                    agg.get("validation_accuracy") or {}
                ).get("ci_low"),
                "validation_accuracy_ci_high": (
                    agg.get("validation_accuracy") or {}
                ).get("ci_high"),
                "validation_loss_mean": (agg.get("validation_loss") or {}).get("mean"),
                "validation_loss_ci_low": (agg.get("validation_loss") or {}).get(
                    "ci_low"
                ),
                "validation_loss_ci_high": (agg.get("validation_loss") or {}).get(
                    "ci_high"
                ),
                "macro_precision_mean": (
                    (agg.get("macro_avg") or {}).get("precision") or {}
                ).get("mean"),
                "macro_precision_ci_low": (
                    (agg.get("macro_avg") or {}).get("precision") or {}
                ).get("ci_low"),
                "macro_precision_ci_high": (
                    (agg.get("macro_avg") or {}).get("precision") or {}
                ).get("ci_high"),
                "macro_recall_mean": (
                    (agg.get("macro_avg") or {}).get("recall") or {}
                ).get("mean"),
                "macro_recall_ci_low": (
                    (agg.get("macro_avg") or {}).get("recall") or {}
                ).get("ci_low"),
                "macro_recall_ci_high": (
                    (agg.get("macro_avg") or {}).get("recall") or {}
                ).get("ci_high"),
                "macro_f1_mean": ((agg.get("macro_avg") or {}).get("f1") or {}).get(
                    "mean"
                ),
                "macro_f1_ci_low": ((agg.get("macro_avg") or {}).get("f1") or {}).get(
                    "ci_low"
                ),
                "macro_f1_ci_high": ((agg.get("macro_avg") or {}).get("f1") or {}).get(
                    "ci_high"
                ),
                "auroc_macro_mean": (agg.get("auroc_macro_mean") or {}).get("mean"),
                "auroc_macro_ci_low": (agg.get("auroc_macro_mean") or {}).get("ci_low"),
                "auroc_macro_ci_high": (agg.get("auroc_macro_mean") or {}).get(
                    "ci_high"
                ),
            }
        )

    # Determine all fieldnames present across rows
    fieldnames = set()
    for r in rows:
...
def _write_csv_cross_validation(
    csv_path: str,
    models_to_folds: Dict[str, List[Dict[str, Any]]],
    aggregates: Dict[str, Dict[str, Any]],
) -> None:
    rows: List[Dict[str, Any]] = []
    for model_name, folds in models_to_folds.items():
        for idx, d in enumerate(folds):
            metrics = d.get("metrics", {})
            report = metrics.get("classification_report", {})
            macro = _extract_macro_metrics(report)
            auroc_list = metrics.get("auroc", []) or []
